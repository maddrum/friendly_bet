...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for bonus_summary in UserBonusSummary.objects.all().prefetch_related(
            "user", "event"
        ):
            ranklist_object, created = UserScore.objects.get_or_create(
                user=bonus_summary.user, event=bonus_summary.event
            )
            if ranklist_object.bonus_points_added:
                self.stdout.write(
                    self.style.WARNING(f"[INFO] skipped for {bonus_summary.user}")
                )
                continue
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"[INFO] Added bonus points for {bonus_summary.user}"
                )
            )
