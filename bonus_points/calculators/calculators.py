...
def predictions_for_all(user, bonus, *args, **kwargs):
    ...
    if total_match_count == total_user_predictions:
        bonus_obj, created = AutoBonusUserScore.objects.get_or_create(
            user=user, bonus=bonus
        )
        bonus_obj.points_gained = bonus.points
        bonus_obj.summary_text = (
            f"Дал прогноза за всеки мач и взел {bonus.points} точки."
        )
        bonus_obj.save()
...
def prediction_for_first_match(user, bonus, *args, **kwargs):
    ...
    check_prediction = UserPrediction.objects.filter(
        user=user, match=first_match
    ).exists()
    if check_prediction:
        bonus_obj, created = AutoBonusUserScore.objects.get_or_create(
            user=user, bonus=bonus
        ...
        bonus_obj.points_gained = bonus.points
        bonus_obj.summary_text = (
            f"Дал прогноза за първия мач и взел {bonus.points} точки."
        )
        bonus_obj.save()
        return f"added {bonus.points} points to user: {user} for bonus: {bonus}"
