...
class BonusDescription(models.Model):
    ...
    available_choices = models.CharField(
        max_length=600, blank=True, null=True, help_text="CSV of available choices"
    )
    bonus_input = models.CharField(max_length=20, default="text", choices=INPUT_CHOICES)

    auto_bonus = models.BooleanField(
        default=False,
        help_text="If this is checked, all users will be included to "
        "participate in this bonus by default.",
    )
    auto_bonus_calculator_function_name = models.CharField(
...
class BonusUserPrediction(models.Model):
    ...
    bonus = models.ForeignKey(
        BonusDescription, on_delete=models.CASCADE, related_name="bonus"
    )
    user = models.ForeignKey(
        get_user_model(), on_delete=models.CASCADE, related_name="bonus_user"
    )
...
class BonusUserScore(models.Model):
    prediction = models.ForeignKey(
        BonusUserPrediction, on_delete=models.CASCADE, related_name="prediction"
    )
...
class AutoBonusUserScore(models.Model):
    user = models.ForeignKey(
        get_user_model(), on_delete=models.CASCADE, related_name="auto_bonus_user"
    )
    bonus = models.ForeignKey(
        BonusDescription, on_delete=models.CASCADE, related_name="auto_bonus_obj"
    )
    points_gained = models.IntegerField(default=0)
    summary_text = models.CharField(max_length=500)
