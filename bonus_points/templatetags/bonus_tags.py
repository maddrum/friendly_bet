...
def check_for_new_bonuses(context):
    ...
    bonus_check = BonusDescription.objects.filter(
        created_on__gte=now_minus_days, bonus_active=True
    ).order_by("-created_on")
    if bonus_check.exists():
        if LAST_BONUS_VISIT_TIME_KEY in context.request.session:
            last_time_visit = datetime.datetime.fromisoformat(
                context.request.session[LAST_BONUS_VISIT_TIME_KEY]
            )
