...
class MatchResultAdmin(ReadOnlyFields):
    readonly_fields = ('created_on', 'edited_on')
    list_display = (
        'match', 'match_is_over', 'match_state', 'score_home', 'score_guest',
        'penalties', 'score_after_penalties_home', 'score_after_penalties_guest',
    )
    ...
    def render_change_form(self, request, context, *args, **kwargs):
        if context['adminform'].form.instance.pk is None:
            context['adminform'].form.fields['match'].queryset = Match.objects.all().exclude(
                match_result__match_is_over=True).order_by('match_start_time')
