...
from events.model_factories import EventMatchStateFactory, EventPhaseFactory, TeamFactory
    ...
    def match_start_time(self):
        ...
        current_counter = (
            Match.objects.filter(
                ...
                match_start_time__lte=datetime.datetime.combine(start_date, datetime.time(23, 59, 59)),
            ).count()
            + 1
        )
...
class MatchResultFactory(DjangoModelFactory):
    class Meta:
        model = MatchResult

    match = factory.SubFactory(MatchFactory)
    match_state = factory.SubFactory(EventMatchStateFactory)
