    ...
    def get_queryset(self):
        ...
        queryset = (
            UserPrediction.objects.filter(
                match__pk=self.kwargs["pk"], match__match_result__match_is_over=True
            )
            .order_by("user__pk")
            .prefetch_related("match")
            .select_related("match__match_result")
        )
        return queryset
...
class ScheduleView(ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        date_bounds = [
            datetime.datetime.combine(current_date, datetime.time(0, 0, 1)),
        ...
        today_matches = all_matches.filter(
            match_start_time__gte=date_bounds[0], match_start_time__lte=date_bounds[1]
        )
        match_order = {}
