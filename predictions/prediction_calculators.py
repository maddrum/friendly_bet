...
from predictions.prediction_scores import (
    GUESSED_MATCH_RESULT,
    GUESSED_MATCH_STATE,
...
def calculate_user_predictions(instance_id=None):
    if instance_id is None:
        ...
        try:
            ...
            return
        queryset = (
            UserPrediction.objects.filter(match=match_result_instance.match)
            .prefetch_related("match__match_result")
            .select_related("match")
            .select_related("bet_points")
        )
...
def calculate_ranklist(instance_id=None):
    ...
    if instance_id is None:
        all_predictions = (
            UserPrediction.objects.filter(match__match_result__match_is_over=True)
            .prefetch_related("prediction_points")
            .select_related("match__phase__event")
        )
    else:
        all_predictions = (
            UserPrediction.objects.filter(match__match_result__pk=instance_id)
            .prefetch_related("prediction_points")
            .select_related("match__phase__event")
        )
